...
from src.inferer.DDPM_Inferer import DiffusionInferer
from generative.networks.nets import DiffusionModelUNet
from src_vqvae.networks.vqvae.configure import get_vqvae_network
...
from src.networks.ddpm_unet import DiffusionWrapper
...
class BaseTrainer:
    def __init__(self, args):
        ...
        if args.vqvae_checkpoint:
            ...
            self.vqvae_model = get_vqvae_network(config=args).to(self.device)

            vqvae_checkpoint = torch.load(vqvae_checkpoint_path)["network"]
            self.vqvae_model.load_state_dict(vqvae_checkpoint)
            self.vqvae_model.to(self.device)
            self.vqvae_model.eval()
        else:
            self.vqvae_model = PassthroughVQVAE()

        if args.vqvae_checkpoint_conditioning:
            vqvae_checkpoint_path = Path(args.vqvae_checkpoint_conditioning)
            # vqvae_config_path = vqvae_checkpoint_path.parent / "vqvae_config.json"
            if not vqvae_checkpoint_path.exists():
                raise FileNotFoundError(f"Cannot find VQ-VAE checkpoint {vqvae_checkpoint_path}")
            # if not vqvae_config_path.exists():
            #     raise FileNotFoundError(f"Cannot find VQ-VAE config {vqvae_config_path}")
            self.vqvae_model_2 = get_vqvae_network(config=args, conditioning_network=True).to(self.device)
            vqvae_checkpoint = torch.load(vqvae_checkpoint_path)["network"]
            self.vqvae_model_2.load_state_dict(vqvae_checkpoint)
            self.vqvae_model_2.to(self.device)
            self.vqvae_model_2.eval()
        else:
            self.vqvae_model_2 = PassthroughVQVAE()

        self.model = DiffusionWrapper(args=args).to(self.device)
        ...
        self.image_size = args.image_size if args.image_size else args.image_size
        self.dynamic_latent_pad = args.dynamic_latent_pad
        if args.latent_pad:
