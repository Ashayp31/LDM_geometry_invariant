...
import ast
import os
from src.trainers import DDPMInfererSuperRes
...
def parse_args():
    ...
    parser.add_argument("--image_conditioning_file", default=None, help="Conditioning by Image file locations")
    parser.add_argument("--image_conditioning", default=False, help="Conditioning by image")
    ...
    parser.add_argument(
        "--vqvae_checkpoint_conditioning",
        default=None,
        help="Path to a VQ-VAE model checkpoint, if you wish to train an LDM. For second image conditioning",
    )

    parser.add_argument("--vqvae_network_conditioning", default="baseline_vqvae")
    parser.add_argument("--use_subpixel_conv_conditioning", default= False)
    parser.add_argument("--no_levels_conditioning", default=3)
    parser.add_argument("--no_res_layers_conditioning", default=3)
    parser.add_argument("--no_channels_conditioning", default=256)
    parser.add_argument("--num_embeddings_conditioning", default=(512,))
    parser.add_argument("--embedding_dim_conditioning", default=(8,))
    parser.add_argument("--dropout_penultimate_conditioning", default=True)
    parser.add_argument("--dropout_enc_conditioning", default=0.0)
    parser.add_argument("--dropout_dec_conditioning", default=0.0)
    parser.add_argument("--output_act_conditioning", default=None)
    parser.add_argument("--downsample_parameters_conditioning", default=(
        (4, 2, 1, 1),
        (4, 2, 1, 1),
        (4, 2, 1, 1),
    ))
    parser.add_argument("--upsample_parameters_conditioning", default=(
        (4, 2, 1, 0, 1),
        (4, 2, 1, 0, 1),
        (4, 2, 1, 0, 1),
    ))
...
if __name__ == "__main__":
    ...
    inferer = DDPMInfererSuperRes(args)
    inferer.inference()
