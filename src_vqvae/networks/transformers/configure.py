...
from src_vqvae.networks.transformers.performer import Performer
...
def get_transformer_network(config: dict) -> TransformerBase:
    if config["vqvae_network"] and config["use_vqvae_aug_conditionings"]:
        conditioning_num_tokens = [2] * len(config["vqvae_augmentations"])
    else:
    ...
    if "performer" in config["network"]:
        network = Performer(
            causal=True,
            ordering=config["ordering"],
            fixed_ordering=config["fixed_ordering"],
            num_tokens=config["vocab_size"] + 1,
            max_seq_len=np.prod(config["spatial_shape"]) + 1 if config["max_seq_len"] is None else config["max_seq_len"],
            dim=config["n_embd"],
            ...
            absolute_position_emb=config["position_emb"]=="absolute",
            spatial_position_emb=config["spatial_position_emb"],
            spatial_shape=config["spatial_shape"],
            conditioning_num_tokens=conditioning_num_tokens,
            conditioning_type=config["conditioning_type"],
            num_tokens_enc=config["vocab_size_enc"] + 1 if config["conditioned_encoding"]
            else 0,
            cross_attend=config["conditionings"] is not None or config["spatial_encoding"],
