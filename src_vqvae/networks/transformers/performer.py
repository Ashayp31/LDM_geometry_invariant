from src_vqvae.utils.constants import (
    TransformerConditioningType,
    TransformerSpatialConditioningType,
)
from src_vqvae.utils.transformer import (
...
from src_vqvae.networks.transformers.img2seq_ordering import Ordering
from src_vqvae.networks.transformers.transformer import TransformerBase
...
class Performer(TransformerBase):
    ...
    def __init__(
        ...
        *,
        ...
        absolute_position_emb: bool = True,
        ...
        assert (
            ...
            f"{rotary_position_emb} {fixed_pmax_seq_lenosition_emb} and {axial_position_emb}."
        )
        ...
        if spatial_position_emb:
            ...
            for i in axis:
                ...
                if (
                    ...
                    == TransformerSpatialConditioningType.FIXED.value and fixed_ordering
                ):
                    ...
                    spatial_position_emb
                    == TransformerSpatialConditioningType.ABSOLUTE.value and fixed_ordering
                ):
                    self.spatial_position_emb.append(
                        AbsoluteSpatialPositionalEmbedding(
                            dim=dim, spatial_indices_sequence=spatial_indices_sequence
        ...
        if spatial_encoding:
            self.spatial_xattention_embedding = nn.Embedding(spatial_buckets, dim)
            self.cross_attend=True
    ...
    def forward(
        ...
        spatial_xatt_conditioning: torch.tensor = None,
        return_encodings: bool = False,
        context_mask: torch.tensor = None,
        reduced_spatial_conditioning: torch.tensor = None,
        **kwargs,
    ):
        ...
        if spatial_xatt_conditioning is not None:
            if reduced_spatial_conditioning is not None:
                spatial_context = self.spatial_xattention_embedding(reduced_spatial_conditioning)
            else:
                spatial_context = self.spatial_xattention_embedding(spatial_xatt_conditioning)
            x += spatial_context
        ...
        if (self.conditioning_type != TransformerConditioningType.CROSSATTEND.value) or (conditionings is None):
            ...
            elif spatial_xatt_conditioning is not None:
                spatial_context = self.spatial_xattention_embedding(spatial_xatt_conditioning)
                kwargs["context"] = spatial_context
