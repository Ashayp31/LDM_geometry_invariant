...
class Quantizer_impl(nn.Module):
    ...
    def embed(self, embedding_indices: torch.Tensor) -> torch.Tensor:
        quantized_st = self.embedding(embedding_indices).permute(0, 4, 1, 2, 3).contiguous()

        return quantized_st


class Quantizer(nn.Module):
    def __init__(self, n_embed, embed_dim, commitment_cost=0.25, decay=0.99, eps=1e-5, use_coordconv=False):
        super().__init__()
...
class BaselineVQVAE(VQVAEBase, nn.Module):
    def __init__(
        ...
        self.dropout_dec_end = dropout_dec
        self.dropout_enc_end = dropout_enc
        if dropout_at_end:
            self.dropout_dec_main = 0.0
            self.dropout_enc_main = 0.0
        else:
            self.dropout_dec_main = dropout_dec
            self.dropout_enc_main = dropout_enc
        self.apply_coordConv = False if apply_coordConv == "False" else True
    ...
    def construct_encoder(self) -> nn.ModuleList:
        ...
        for i in range(self.n_levels):
            in_channels = 1 if i == 0 and not self.apply_coordConv else 4 if i == 0 and self.apply_coordConv else self.n_channels // 2
            out_channels = self.n_channels // (1 if i == self.n_levels - 1 else 2)

            modules.append(
                nn.Conv3d(
                    in_channels=1 if i == 0 and not self.apply_coordConv else
                                4 if i == 0 and self.apply_coordConv else
                                self.n_channels // 2,
        ...
        modules.append(
            nn.Dropout3d(self.dropout_enc_end))
        modules.append(
    ...
    def construct_decoder(self) -> nn.ModuleList:
        ...
        modules = [nn.Conv3d(self.embed_dim,
                             self.n_channels, 3, stride=1, padding=1)]
        ...
        for i in range(self.n_levels):
            ...
            if i == self.n_levels - 1:
                modules.append(nn.Dropout3d(self.dropout_dec_end))
            modules.append(
                SubpixelUpsample(
                    dimensions=3,
    ...
    def decode_samples(self, embedding_indices: List[torch.Tensor], coords: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, images: List[torch.Tensor]) -> Dict[str, List[torch.Tensor]]:
