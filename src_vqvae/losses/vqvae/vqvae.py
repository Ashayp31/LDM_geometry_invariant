...
from src_vqvae.handlers.general import TBSummaryTypes
...
class JukeboxPerceptualLoss(torch.nn.Module):
    ...
    def _calculate_fake_3d_loss(
        ...
        # indices = indices.to(dtype=torch.long, device=y_pred_slices.device)
        # print(indices.dtype)
        indices = indices.int()

        y_pred_slices_new = torch.zeros((len(indices), y_pred_slices.shape[1], y_pred_slices.shape[2],
                                         y_pred_slices.shape[3]), device=y_pred_slices.device)
        y_slices_new = torch.zeros((len(indices), y_pred_slices.shape[1], y_pred_slices.shape[2],
                                         y_pred_slices.shape[3]), device=y_pred_slices.device)

        for i in range(len(indices)):
            y_pred_slices_new[i] = y_pred_slices[indices[i]]
        for i in range(len(indices)):
            y_slices_new[i] = y_slices[indices[i]]
        y_pred_slices = y_pred_slices_new
        y_slices = y_slices_new
