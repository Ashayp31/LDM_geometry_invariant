...
from src_vqvae.transforms.general.dictionary import TraceTransformsd
from src_vqvae.utils.constants import VQVAEModes, AugmentationStrengthScalers
...
class RandResizeImg(MapTransform):
    ...
    def __call__(self, data):
        ...
        if rand_val < 0.8:
            ...
            y_len = img.shape[2]
            z_len = img.shape[3]
            new_x_len = int(8*((x_len/rand_resolution_change)//8))
            new_y_len = int(8*((y_len/rand_resolution_change)//8))
            new_z_len = int(8*((z_len/rand_resolution_change)//8))
...
class CheckDimensions(MapTransform):
    ...
    def __call__(self, data):
        ...
        x_rem = 8-(x_len % 8) if ((x_len % 8) != 0) else 0
        y_rem = 8-(y_len % 8) if ((y_len % 8) != 0) else 0
        z_rem = 8-(z_len % 8) if ((z_len % 8) != 0) else 0
    ...
    if mode == VQVAEModes.DECODING.value:
        ...
        if is_augmented:
            if patch_size:
                ...
                augmentations += [
                    RandAdjustContrastd(keys=[key], prob=0.2, gamma=(0.95, 1.05)),
                    RandShiftIntensityd(keys=[key], prob=0.2, offsets=(0.0, 0.025)),
                    RandGaussianNoised(keys=[key], prob=0.2, mean=0.0, std=0.001),
                    RandGaussianSmoothd(keys=[key], prob=0.0, sigma_x=(0.05, 0.3), sigma_y=(0.05, 0.3),
                                        sigma_z=(0.05, 0.3))]
