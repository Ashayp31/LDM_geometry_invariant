...
from src_vqvae.utils.constants import VQVAEModes, TransformerModes
from src_vqvae.utils.vqvae import get_transformations as get_vqvae_transformations

from src_vqvae.networks.transformers.img2seq_ordering import (
    Ordering,
    OrderingType,
...
def get_subjects(
    subjects_file_path: Union[str, Tuple[str, ...]],
    ...
    if isinstance(subjects_file_path, str):
        subjects_file_path = [subjects_file_path]
    else:
        subjects_file_path = list(subjects_file_path)
    ...
    # for file in zip(subjects_files) if is_multi_level else subjects_files[0]:
    # for file in subjects_files[0]:
    # for file in subjects_files:
            ...
            if spatial_encodings_file_path:
                ...
                try:
                    ...
                    print("Cannot find Spatial Encoding npy file for for ", file)
                    mia_subjects += 1
                    valid_subject = False
                    continue
...
def get_data_flow(
    ...
    training_transform, vqvae_augmentations = get_transformations(
        ...
        vqvae_transformations_config={
            ...
            "crop_path": config["cropping_file"],
            "crop_type": config["cropping_type"],
...
def prepare_batch(
    ...
    vqvae_cond_network: VQVAEBase = None,
    vqvae_net_level: int = 0,
    vqvae_augmentations: Compose = None,
    use_vqvae_aug_conditionings: bool = False,
    vqvae_augmentations_conditionings_key: str = "quantization_trace_dict",
    ...
    if encoded.shape[1] > 21504:
        encoded = encoded[:,:21504]

    encoded = F.pad(encoded, (1, 0), "constant", 0)
    encoded = encoded.long()

    if random.uniform(0,1) > 0.7:
        token_mask = torch.zeros_like(encoded).bool()
        if not inference:
    ...
    #    if token_mask.shape[1] > 21504:
    #        token_mask = token_mask[:,:21504]
    #    token_mask = F.pad(token_mask, (1, 0), "constant", 0)
    #    token_mask = 1-token_mask
    ...
    if conditioned_encoding:
        ...
        if vqvae_cond_network:
            conditioning_encoding = vqvae_cond_network.index_quantize(conditioning_encoding)[vqvae_net_level]
        ...
        if conditioning_encoding.shape[1] > 21504:
            conditioning_encoding = conditioning_encoding[:,:21504]
        conditioning_encoding = F.pad(conditioning_encoding, (1, 0), "constant", 0)
    ...
    if vqvae_network and use_vqvae_aug_conditionings:
        ...
        if "quantization_trace_dict" in batch.keys():
            trace = batch[vqvae_augmentations_conditionings_key]

            for transform_name, boolean_value in trace.items():
                if len(boolean_value.shape) == 1:
                    boolean_value = boolean_value[..., None]

                boolean_value = boolean_value.long()
            ...
            for i in range(len(vqvae_augmentations.transforms)):
                ...
                boolean_value = torch.tile(boolean_value, (encoded.shape[0], 1))
                boolean_value = convert_tensor(boolean_value, device, non_blocking)
                conditioned.append(boolean_value)
    ...
    if spatial_conditioning:
        ...
        if spatial_encoding.shape[1] > 21504:
            spatial_encoding = spatial_encoding[:,:21504]
    ...
    if original_nii_path is not None:
        ...
        return (x_input, x_encoding, conditioned, token_mask_input, spatial_encoding, x_original_input), x_target
    else:
        return (x_input, x_encoding, conditioned, token_mask_input, spatial_encoding), [x_target, token_mask_loss]
