...
def create_folder_structure(config: dict, logger: Logger):
    ...
    config["outputs_directory"] = (
        outputs_directory
        if config["outputs_directory"] is None
        else config["outputs_directory"]
    )

    Path(config["outputs_directory"]).mkdir(parents=True, exist_ok=True)
