...
class TransformerTrainingInferer(Inferer):
    ...
    def __call__(
        ...
        if len(inputs) == 2:
            ...
            sequence_out, sequence_in, conditioning, token_mask, spatial_enc = inputs 
            return network(sequence_out, sequence_in, conditioning, token_mask, spatial_enc, *args, **kwargs).transpose(1, 2)
        else:
            sequence_out, sequence_in, conditioning, token_mask, spatial_enc, original_pet = inputs 
            return network(sequence_out, sequence_in, conditioning, token_mask, spatial_enc, *args, **kwargs).transpose(1, 2)
