...
from src.utils.constants import VQVAEModes
...
def run(
    ...
    mode: str = "training",
    cropping_file: str = None,
    cropping_type: str = None,
    apply_coordConv: bool = False,
    input_has_coordConv: bool = False,
    ...
    decay_warmup: str = "step",
    max_decay_epochs: Union[str, int] = 50,
    norm: str = None,
    dropout_penultimate: bool = False,
    ...
    Args:
        ...
        outputs_directory (str): Overwrite the automated outputs directory. This is mainly intended to be used during
            decoding mode so the decoded samples are saved in the same folder with their quantized representations.
        ...
        cropping_file (str) : Location of file that shows the crop ratio's along each dimension. The file should have
                            7 Columns, subject, x0, x1, y0, y1, z0, z1 with values between 0 and 1.
                            
        cropping_type (str) : Type of cropping to be used on the data. None responds to no cropping,
                        'with_padding' implies random cropping that pad the image with blank space to the original size,
                        'without_padding' implies random cropping with no image padding i.e. new size image,
                        if cropping_file is not None, the above crops will be pre-specified by the file,
                        Defaults to None.

        apply_coordConv (bool) : Whether or not to apply coordconv to data in the VQVAE.
        ...
        normalize (bool): Whether to normalize the input data in the 0-1 range. normalize and standardize can't be
            turned on simultaneously. Defaults to True.

        standardize (bool): Whether to standardize the input data. normalize and standardize can't be turned on
        ...
        num_samples (int): The number of patches to be extracted from each samples. Defaults to 1.
        ...
        eval_num_samples (int): The number of patches to be extracted from each sample for evaluation. Defaults to 1.
        ...
        network (str): What network to use. Defaults to 'baseline_vqvae'.
        ...
        decay_warmup (Union[str, Tuple[str,...]): Dictates the behaviour of the deacy warmup. It can be either a string
            which will apply the same strategy to all decays or a tuple indicating each decay a strategy. The strategies
            are:
                "step" will increase the decay in 4 qually spaced steps from decay to 0.99.
                "linear" will linearly increase the decay from decay to 0.99 over max_decay_epochs.
                "none" will not use any warmup.

        max_decay_epochs (Union[int, Tuple[int,...]]): After how many epochs the Exponential Moving Average decay goes
            from decay to 0.99. It can be personalized for each quantization level. The count of max_decay_epochs must
            be the same as decay. If the decay_warmup for a certain level is "none" the value will just be ignored.
            Defaults to 100.

        norm (str): Which normalization technique the network will use. Defaults to None.

        dropout (float): The amount of dropout udse in the network. Defaults to 0.1
